...
class Statuses_model extends Model
{
    ...
    /**
     * Return ONLY specific Order statuses (non-destructive, Orders-only use).
     * @param array|null $names whitelist of status_name values, preserve list order when possible
     * @return array id => name
     */
    public static function getDropdownOptionsForOrderLimited(array $names = null)
    {
        $names = $names ?: ["Received","Preparation","Delivery","Paid"];

        // Base: order-only statuses
        $query = static::isForOrder()->whereIn("status_name", $names);

        // Try to preserve whitelist order first, then fall back to DB priority
        // FIELD() is MySQL/MariaDB; if unsupported, it is safe to ignore and priority ordering remains.
        $placeholders = implode(",", array_map(fn($n)=>"'".str_replace("'","\\'",$n)."'", $names));
        try {
            $query = $query->orderByRaw("FIELD(status_name, $placeholders)");
        } catch (\Throwable $e) {
            $query = $query->orderBy("priority"); // fallback
        }

        // dropdown(column) returns [id=>label]
        $opts = $query->dropdown("status_name");

        // If any of the requested names are missing in DB, we just return the ones that exist.
        return $opts;
    }
}
