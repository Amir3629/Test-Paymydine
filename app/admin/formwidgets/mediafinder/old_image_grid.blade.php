...
document.addEventListener("DOMContentLoaded", function () {
    let imgElement = document.querySelector("#mediafinder-formloaderlogo-loader-logo img");

    if (imgElement) {
        let imagePath = imgElement.getAttribute("src");
        let currentUrl = new URL(window.location.href);
        let currentSrs = currentUrl.searchParams.get("srs");

        // Always update 'srs' if it's different or missing
        if (!currentSrs || currentSrs !== imagePath) {
            currentUrl.searchParams.set("srs", imagePath);
