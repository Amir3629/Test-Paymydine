...
for T in ti_tables ti_orders ti_locationables ti_waiter_calls ti_waiter_requests; do
  mysql -u "$DB_USER" -p"$DB_PASS" -e "USE $DB_NAME; SHOW CREATE TABLE ${T}\G" > "$OUT/db/CREATE_${T}.txt" || true
done
...
mysql -u "$DB_USER" -p"$DB_PASS" -e "USE $DB_NAME; \
  SELECT o.order_id,o.order_type,o.table_id,o.payment_status,o.created_at, t.table_no,t.table_name \
...
mysql -u "$DB_USER" -p"$DB_PASS" -e "USE $DB_NAME; \
  SELECT * FROM ti_locationables WHERE locationable_type='tables' ORDER BY id DESC LIMIT 30;" \
...
grep -Rn "Schema::create\|Blueprint" database/migrations > "$OUT/backend/grep_migrations.txt" || true
